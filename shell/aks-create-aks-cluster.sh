...
# AKSクラスタをデプロイ。まずは最小ノード数(1)で作成する。

# IKOをここ(nodepool1)で動かしたいので、--nodepool-taints CriticalAddonsOnly=true:NoScheduleを除去
az aks create -g $aksrg --name $aksname --node-count 1 --generate-ssh-keys --node-vm-size Standard_D2s_v5
...
az aks nodepool add --resource-group $aksrg --cluster-name $aksname --mode user --name arbiter --node-count 1 --zones 3 --node-vm-size Standard_D2s_v5 --labels speedtest/node-type=iris-arbiter
